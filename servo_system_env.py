...
class ServoSystemEnv(gym.Env):
	def __init__(self):
		...
		self.rad_last = 0.0  # Speed at the previous moment 前一时刻的速度
		self.rad_cur = 0.0  # Speed at the moment  当前时刻的速度
		self.rad_next = 0.0  # Speed at the next moment  下一时刻的速度
		self.rad_next_next = 0.0  # Speed at the next next moment  下下时刻的速度
		self.error_last = 0.0  # The error at the previous moment  上一时刻的误差
		self.error_cur = 0.0  # The error at the moment  当前时刻的误差

		self.ec_last = 0.0  # electric current at the previous moment  上一时刻的电流
		self.cnt = 0  # used to count the step number 记录步数的变量

		self.rad_threshold = 9000  # 速度阈值
	...
	def _get_eletric_immediate_reward(self, error, electric):
		"""
		Design the Reward Function of search immediate reward.  设计搜索的即时奖励功能
		:param error:
		:param electric:
		:return:
		"""
	...
	def reset(self):
		"""
		# 初始化环境 返回一个 []
		:return: [rad_last, rad_cur, rad_next, rad_next_next, error_last, error_cur]
		"""

		self.rad_last = 0.0  # Speed at the previous moment 上一时刻的速度
		self.rad_cur = 0.0  # Speed at the moment  当前时刻的速度
		self.rad_next = 0.0  # Speed at the next moment 下一时刻的速度
		self.rad_next_next = 0.0  # Speed at the next next moment  下下时刻的速度
		self.error_last = 0.0  # The error at the previous moment  上一时刻的误差
		...
		if args.choose_model == "search_pid_parameter":
			self.pid = PID(kp=args.kp, ki=args.ki, kd=args.kd, args=args)  # 寻找pid参数
		else:
			self.pid = PID(kp=args.kp_rl, ki=args.ki_rl, kd=args.kd_rl, args=args)
